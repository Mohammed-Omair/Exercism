...
export function fixListOfWagons(ids) {
  const [first, second, ...elements] = ids
...
export function correctListOfWagons(ids, missingWagons) {
  const [first, ...elements] = ids
  return [first, ...missingWagons, ...elements]
}
...
export function separateTimeOfArrival(information) {
  const {timeOfArrival, ...others} = information
  return [timeOfArrival, others]
}
